...
const transformGraphEventForComparison = (
  graphEvent: CalendarData,
): CalendarEvent | null => {
  try {
    ...
    return {
      id: graphEvent.id || Math.random().toString(), // Fallback to random string if no id
...
export const CalendarComparison: React.FC<CalendarComparisonProps> = ({
  ...
  // Initialize user calendars with loading state, always including current user
  useEffect(() => {
    ...
    // Create GraphUser object for current user
    const currentUserAsGraphUser: GraphUser = {
      displayName: currentUser.displayName,
      userPrincipalName: currentUser.userPrincipalName,
      id: currentUser.userId, // Use userId as id
    };
    // Combine current user with selected users, ensuring current user is first and not duplicated
    const allUsers = [currentUserAsGraphUser];
    selectedGraphUsers.forEach((user) => {
      // Only add if not already the current user
      if (user.userPrincipalName !== currentUser.userPrincipalName) {
        allUsers.push(user);
      }
    });

    const initialUserCalendars: UserWithCalendar[] = allUsers.map(
      (user, index) => {
  ...
  }, [selectedGraphUsers, currentUser]);
      ...
      const updatedCalendars = await Promise.all(
        userCalendars.map(async (userCal) => {
          try {
            ...
            const graphEvents = await fetchCalendarEventsForToday(
              userCal.user.userPrincipalName,
            );

            console.log(
              `Fetched ${graphEvents.length} raw events for ${userCal.user.displayName}`,
            );
            ...
            console.log(
              `Transformed ${transformedEvents.length} events for ${userCal.user.displayName}`,
            );
            ...
            console.error(
              `Error fetching events for ${userCal.user.displayName}:`,
              error,
            );
            return {
              ...userCal,
  ...
  const handleRefreshEvents = () => {
    ...
    setUserCalendars((prev) =>
      prev.map((uc) => ({ ...uc, loading: true, error: undefined })),
    );
  ...
  const totalEvents = userCalendars.reduce(
    (sum, uc) => sum + uc.events.length,
    0,
  );
